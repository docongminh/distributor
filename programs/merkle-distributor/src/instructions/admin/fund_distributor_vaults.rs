...
#[derive(Accounts)]
pub struct FundDistributorsVault<'info> {
    /// The [ParentState].
...
pub fn handle_fund_distributors_vault<'c: 'info, 'info>(
    ...
    // [distributor, distributor_vault, distributor, distributor_vault]
    ...
    for pair_accounts in remaining_accounts.chunks(2) {
        let distributor_account = AccountLoader::<MerkleDistributor>::try_from(&pair_accounts[0])?;
        let distributor_vault = Account::<TokenAccount>::try_from(&pair_accounts[1])?;
